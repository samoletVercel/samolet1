...
const BrandingProjectPage = async ({ params }: { params: { id: number } }) => {
        ...
        project.post.content[i].innerBlocks.forEach((el: { innerBlocks: any; }) => {
          if (el.innerBlocks[0].blockName == 'my-blocks/image-block') {
            if (el.innerBlocks[0].attrs.type === "image") {
              ...
              innerBlocks.push(<div key={Math.random()}><video width="680" src={el.innerBlocks[0].attrs.src} height="400" muted preload="none" controls>
              Your browser does not support the video tag.
            </video></div>)
            }
          } else {
